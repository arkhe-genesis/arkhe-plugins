            ...
            "/desci-publish" | "desci-publish" => {
                ...
                while i < parts.len() {
                    match parts[i] {
                        ...
                        _ => {
                            if title.is_empty() && !parts[i].starts_with("--") {
                                title = parts[i..].join(" ");
                                break;
                            }
                            i += 1;
            ...
            "/desci-royalties" | "desci-royalties" => {
                ...
                while i < parts.len() {
                    match parts[i] {
                        ...
                        "--split" => {
                            if i + 1 < parts.len() {
                                let split_parts: Vec<&str> = parts[i + 1].split(':').collect();
                                if split_parts.len() == 2 {
                                    let npub = split_parts[0].to_string();
                                    let share = split_parts[1].parse::<f32>().unwrap_or(0.0);
    ...
    pub async fn execute(&self, orchestrator: &mut SecondSelfOrchestrator) -> Result<String, String> {
        match self {
            Self::Publish { title, spdx_license, copyright_holder, software_version, derived_from, ai_generated, training_data, .. } => {
                ...
                Ok(format!("✅ Node publicado com título: {}", title))
            }
