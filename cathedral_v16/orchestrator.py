...
class CathedralOrchestrator:
    ...
    async def imagine(self, proposal: ActionProposal) -> ImaginedTrajectory:
        ...
        # Rola imaginação
        features, rewards = self.world_model.imagine_rollout(
            start_state=state,
        ...
        rewards_list = rewards.squeeze(-1).cpu().tolist()
        if rewards_list and isinstance(rewards_list[0], list):
            rewards_list = [r[0] for r in rewards_list]

        for r in reversed(rewards_list):
            cumulative = r + gamma * cumulative
            returns.insert(0, cumulative)

        # Pontuação de segurança da trajetória imaginada (mocked since continues wasn't updated)
        safety_score = 1.0
