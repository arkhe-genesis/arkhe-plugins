...
class VisionEncoder(nn.Module):
    ...
    @torch.no_grad()
    def extract_for_cognition(self, observation: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Método otimizado para inferência pura (sem gradiente)."""
        self.eval()
        cls_token, patch_proj = self.forward(observation)
        return cls_token, patch_proj
