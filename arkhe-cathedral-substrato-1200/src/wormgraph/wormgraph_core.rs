...
impl WormGraph {
    ...
    pub fn export_fair_snapshot(&self) -> FairSnapshot {
        ...
        FairSnapshot {
            ...
            temporal_anchor: Some(alloc::format!("temporal-nonce-{}", self.temporal_nonce)),
        }
    }
    ...
    fn update_context_window(&mut self, node_id: NodeId) -> Result<(), WormGraphError> {
        ...
        while self.current_token_count + estimated_tokens > self.max_context_tokens
            ...
            if let Some(idx) = evict_idx {
                let evicted_id = self.context_window.remove(idx).unwrap();
...
mod tests {
    ...
    fn test_metadata() -> NodeMetadata {
        NodeMetadata {
            ...
            phi_c: 0.99,
            theosis: 0.98,
            tags: alloc::vec![String::from("memory"), String::from("ontological")],
            cross_links: alloc::vec![],
    ...
    fn test_wormgraph_context_eviction() {
        ...
        for i in 0..50 {
            ...
            let embedding = dummy_embedding(768);
            let _ = wg.add_node(&alloc::format!("Content {}", i), metadata.clone(), dna, embedding);
        }
    ...
    fn test_cosine_similarity() {
        let a = alloc::vec![1.0, 0.0, 0.0];
        let b = alloc::vec![1.0, 0.0, 0.0];
        let c = alloc::vec![0.0, 1.0, 0.0];
