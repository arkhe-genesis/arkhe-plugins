...
HAS_V16 = False  # True quando arkhe_data.dll/so está disponível
...
class SafetyEngineZ3:
    ...
    def __init__(self, max_force=10.0, forbidden_targets=None):
        try:
            ...
            logger.warning("SafetyEngineZ3: pacote z3 indisponível, usando stub de aprovação.")
    ...
    def check(self, action: np.ndarray, detections: List[dict]) -> Tuple[bool, str]:
        ...
        if not self.available:
            ...
            return True, "Aprovado por stub (Z3 não instalado)"
...
class EpisodicMemoryHNSW:
    ...
    def __init__(self, dim=288, data_dir="Cathedral/zvec_data/hnsw_fallback",
        ...
        try:
            ...
            self.available = False
            logger.warning("EpisodicMemory: pacote hnswlib indisponível, usando array Python (lento)")
            self.index = []
        ...
        if self.available and os.path.exists(index_file):
    ...
    def store(self, state_vector: np.ndarray, metadata: dict):
        if self.available:
            ...
            self.index.append(state_vector.astype(np.float32))
    ...
    def retrieve(self, state_vector: np.ndarray, top_k: int = 5) -> List[dict]:
        ...
        if self.available:
            ...
            # Slow fallback
            dists = [float(np.sum((state_vector - vec)**2)) for vec in self.index]
            top_indices = np.argsort(dists)[:top_k]
            for idx in top_indices:
                mem = self.memories[idx].copy()
                mem["distance"] = dists[idx]
                results.append(mem)
...
class FastBrain:
    ...
    def __init__(self, config):
        ...
        ) if safety["engine"] in ("z3", "stub") else None
