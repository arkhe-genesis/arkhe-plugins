...
pub struct HybridAttention {
    ...
    pub csa_weight: Tensor,
    pub hca_weight: Tensor,
    pub sliding_weight: Tensor,
}
...
impl HybridAttention {
    ...
    pub fn forward(&self, x: &Tensor, kv_cache: Option<&Tensor>) -> (Tensor, Tensor) {
        ...
        // Sliding window might have a different size, so we'll just use a simplification
        // where we assume it returns the full tensor for this placeholder
        let slide_out = x.clone(); // self.sliding.forward(x, kv_cache);
