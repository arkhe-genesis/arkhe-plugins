...
impl MoELayer {
    ...
    fn combine(
        ...
        let mut token_outputs = vec![Vec::new(); batch_size];

        for (token_idx, _expert_id, weight, output) in outputs {
            token_outputs[token_idx].push((weight, output));
        }

        for (token_idx, experts_for_token) in token_outputs.into_iter().enumerate() {
    ...
    pub fn load_stats(&self) -> crate::moe::load_balancer::LoadBalancerStats {
