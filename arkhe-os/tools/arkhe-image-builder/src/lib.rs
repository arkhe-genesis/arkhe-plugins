...
impl BundleHeader {
    pub fn read<R: Read>(reader: &mut R) -> Result<Self, std::io::Error> {
        ...
        let mut iso_bytes = [0u8; 8];
        reader.read_exact(&mut iso_bytes)?;
        let image_spec_offset = u64::from_le_bytes(iso_bytes);

        let mut iss_bytes = [0u8; 8];
        reader.read_exact(&mut iss_bytes)?;
        let image_spec_size = u64::from_le_bytes(iss_bytes);

        let mut nl_bytes = [0u8; 4];
        reader.read_exact(&mut nl_bytes)?;
        let num_layers = u32::from_le_bytes(nl_bytes);

        let mut lto_bytes = [0u8; 8];
        reader.read_exact(&mut lto_bytes)?;
        let layer_table_offset = u64::from_le_bytes(lto_bytes);
    ...
    pub fn extract_all_layers(&mut self, dest: &Path) -> Result<(), BundleError> {
        ...
        for i in 0..self.layer_table.len() {
            ...
            for entry in walkdir::WalkDir::new(&layer_dir) {
                ...
                let rel_path = entry.path().strip_prefix(&layer_dir).map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
