...
use crate::trinity_core::ConsciousnessState;
...
impl CognitiveRouter {
    ...
    pub async fn combine(
        ...
        match strategy { RouterStrategy::Adaptive => unreachable!(),
            ...
            RouterStrategy::WeightedVoting => {
                ...
                let out_len = outputs.len(); for output in outputs {
                    total_conf += output.confidence;
                    combined_content.push_str(&format!("[{}] {}\n", output.source_expert, output.content));
                    combined_tools.extend(output.tool_calls);
                }

                Ok(vec![CognitiveOutput {
                    content: combined_content,
                    tool_calls: combined_tools,
                    confidence: total_conf / out_len as f64,
                    thinking_trace: None,
            ...
            RouterStrategy::Concatenate => {
                ...
                let mut total_conf = 0.0;
                let out_len_2 = outputs.len(); for output in outputs {
                    combined.content.push_str(&format!("[{}]\n{}\n\n", output.source_expert, output.content));
                    combined.tool_calls.extend(output.tool_calls);
                    total_conf += output.confidence;
                }
                combined.confidence = total_conf / out_len_2 as f64;
                Ok(vec![combined])
            }
        }
    }
