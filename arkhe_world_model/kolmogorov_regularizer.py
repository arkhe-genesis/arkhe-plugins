...
def kolmogorov_regularizer(
    ...
    for name, param in model.named_parameters():
        ...
        # Acumular norma Lp
        if p == 2.0:
            w_norm_p += param.pow(2).sum()
        elif p == 1.0:
            w_norm_p += param.abs().sum()
        else:
            w_norm_p += param.abs().pow(p).sum()

        n_params += param.numel()

    if isinstance(w_norm_p, float):
...
def kolmogorov_complexity_estimate(
    ...
    K_estimate = w_norm_sq * math.log2(w_norm_sq + 1)  # aproximação: ‖θ‖² · log₂(‖θ‖²)
...
class KolmogorovWeightDecay(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            log_term = math.log2(w_norm_sq + 1)  # log₂ aproximado
