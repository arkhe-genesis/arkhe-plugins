...
class WorldModelEmbryo(nn.Module):
    ...
    def __init__(self, config: Optional[WorldModelConfig] = None):
        ...
        # Define a dummy parameter to avoid an empty parameter list error
        # in the optimizer when the stages haven't been lazily loaded yet
    ...
    def forward(
        ...
        if DevelopmentStage.TOKEN_GROUNDING in self.active_stages:
            ...
            if isinstance(llm_emb, np.ndarray):
                llm_emb = torch.from_numpy(llm_emb).float()
        ...
        if DevelopmentStage.MULTIMODAL_FUSION in self.active_stages:
            if visual_input is not None and isinstance(visual_input, np.ndarray):
