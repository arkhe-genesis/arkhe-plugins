...
class ArweaveDataLayer:
    ...
    def fetch_data(self, tx_id: str) -> Dict:
        ...
        try:
            ...
            if "Not Found" in response.text or "Invalid" in response.text or response.text.startswith("<!DOCTYPE html>"):
...
class PermawebBridge:
    ...
    def __init__(self, config: Optional[PermawebConfig] = None):
        ...
        # Add _wallet property
        if self.config.wallet_jwk:
            self._wallet = self.config.wallet_jwk
        elif self.config.wallet_path:
            with open(self.config.wallet_path) as f:
                self._wallet = json.load(f)
        else:
            self._wallet = None
    ...
    def spawn_agent_process(self, agent_id: str,
        ...
        elif "error" in result: # Mock for test isolation
            self._process_registry[agent_id] = {
                "process_id": f"mock-process-{agent_id}",
                "created_at": datetime.now(timezone.utc).isoformat(),
            }
